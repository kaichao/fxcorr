...
| 本目录 | 作用 | 对照源 |
|---|---|---|
| main.cpp | 参数/batch.json 解析、subint 循环驱动 | 骨架参照 core.cpp:694-801（zeroAutocorrelations→setValidFlags→setData→setOffsets→resetpcal→process 循环）；对齐校验为新增（data-spec 12 节） |
...
| fenginewriter.{h,cpp} | .sp / pcal.bin / autocorr.bin 写盘 | 布局照 data-spec 5.3；写入序照 core.cpp:1145-1153（finalisepcal→getPcal）、:1184-1243（averageFrequency→getAutocorrelation） |
...
- **weights 是 per-band**：.sp 的 weights 段写 `getDataWeight(band, subloop)`（perbandweights 或 dataweight），见 data-spec 5.3 修订说明。
...
- 资产在 `fxcorr/test/`：test.vex（上游 tests/Synthetic/test-usb.vex 原版）、test.v2d（2 站 T1/T2）、gen_test_vdif.py（生成 2bit 单 band VDIF；**低位先打包**，对齐 mark5access format_vdif.c lut2bit 位序——datasim 因上游 IPP 依赖无法 --noipp 构建，此脚本是替代品）。
...
- 对拍 mpifxcorr 待 fxcorr-x 完成后用 SWIN 逐记录比较（impl-plan 4.2）。
