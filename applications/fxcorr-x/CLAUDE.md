...
- 读 `workdir/vis/<batch_id>/batch.json`（run_batch.sh 预写），取 start_mjd / n_subints / config_file / difx_dir。
...
- 已验证：2 站 4 秒实验 4 subint（2 积分）SWIN 与 mpifxcorr 逐记录全等（6/6，可见度 <1e-6、weight 逐位一致）；difx2fits 出 FITS 成功。
