...
//
// Timestamps and frame numbers are derived from the batch start second and
// increment monotonically: sec = startsec + n * nsampframe / rate,
// frame = n % (rate / nsampframe).  The constructor rejects band counts that
// are not powers of two (VDIF nchan field) and batches whose duration is not
// an integer number of frames (the global-continuity check for distributed
// generation: a split point that breaks frame numbering would corrupt the
// VDIF stream).
class VDIFWriter
{
public:
	// outpath: full output file path
	// startsec: seconds since the VDIF epoch (2000.0) of the batch start
	...
	VDIFWriter(const std::string &outpath, long long startsec,
