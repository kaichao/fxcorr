...
FreqStationGen::FreqStationGen()
	: numsamps(0), framesinblock(0), framecounter(0), vpsamps(0),
	  noisesigma(0.0), adaptive(false), ready(false)
{
}
                          ...
                          const string &station, int vpsamps_, bool adaptive_)
{
	...
	for(size_t b = 0; b < bandfreqmhz.size(); b++)
	{
		...
		// gain budget); sigma enters as the noise power added to the rms-1
		// common signal
		bd.scale = 0.5 / (sqrt(1.0 + noisesigma * noisesigma) * sqrt((double)bd.blksize));

		bd.temp = new fftwf_complex[bd.blksize];
		bd.baseband = new fftwf_complex[(size_t)((long long)bd.blksize * grid.slicesperblock)];
...
bool FreqStationGen::processBlock(const float *blockdata, long long nfloats)
{
	...
	// inverse DFT to the complex baseband stream (datasim fabricatedata
	// minus the flux/SEFD steps, which are the P2 insertion point)
	for(long long s = 0; s < nslices; s++)
	{
		...
		for(size_t b = 0; b < bands.size(); b++)
		{
			...
			memcpy(bd.baseband + s * bd.blksize, bd.temp,
...
void FreqStationGen::fillFramePayload(unsigned char *payload, int payloadbytes, int nbands)
{
	...
	{
		Band &bd = bands[b];
		// frame chain mirroring datasim subband.cpp processdata (with the
		// identity fractional-sample / fringe steps elided but kept in
		// sequence so P2 fills the marked spots):
		//   DFT(/N) -> DC zero -> IDFT -> DFT(/N) -> Hermitian 2N -> 2N IDFT
		memcpy(bd.procbuf, bd.baseband + framecounter * vpsamps,
		...
		bd.procbuf[0][1] = 0.0f;
		// [P2: fractional sample correction on procbuf]
		fftwf_execute(bd.planbwd);
		// [P2: fringe rotation on procbuf]
		fftwf_execute(bd.planfwd);
		for(int k = 0; k < vpsamps; k++)
		{
			bd.procbuf[k][0] *= (float)invN;
