...
- **记录中断 `FXSIM_GAPS`（2026-09-18 加）**：`vdifwriter` 支持在指定帧号处造记录中断，两种形式——缺口（帧号跳过、不写字节，文件比时间轴**短**）与 filler（额外写帧头全零的帧占位，文件比时间轴**长**），与 t25362 的真实中断同构（见 `fxcorr/reader-model.md`）。**filler 帧数与丢失帧数相互独立**：`:f` 默认一对一，`:f<N>` 单独指定（2026-09-18 晚加）；**占位帧的字节形态也可选**（2026-09-19 加）：`:p<N>` 整帧写 vdifio 的 `FILL_PATTERN`（0x11223344）、`:h<N>` 只有帧首 4 字节是它——这两处正是 `vdifmux` 分别判的地方（帧尾命中跳**整帧**、帧首命中只跳 **8 字节**，`vdifmux.c:598/606`），造出来是为了把 fxcorr 的识别与上游的判据摆在同一组数据上对照（`fxcorr/reader-model.md` 4.8 的形态清单）。三种形态在时间轴上完全等价（都占字节、不占槽、帧号照常跳过缺失帧），只有字节内容不同——t25362 的中断是 81..508 帧 filler 对 10..63 帧缺口，而把读位置带偏的正是这个长度差，一对一写法复现不了（判据 `fxcorr/test/gaps/run_filler.sh`）。**两种形式都必须推进帧号**：中断是"真实过去了的时间"，帧号要跳过丢失的帧数，filler 只是额外留下占位**字节**——首版让 filler 不推进帧号，于是 filler 形式的数据帧号范围比缺口形式短 73 帧，同一段观测被描述成两个长度（`reader-model.md` 的 filler 判定「帧号跳跃只反映真实丢失，与 filler 帧数无关」正是这条）。实现在 `VDIFWriter::writeFrame` 内部（命中 `atframe` 时处理），legacy 与新路径两个调用点自动生效；不设该变量时行为完全不变。检验步骤与验收判据见 `fxcorr/test/gaps/README.md`。
- **记录起点偏移 `FXSIM_STARTOFFSET`（2026-09-19 加，A 类回归）**：跳过 batch 开头的这么多帧（`VDIFWriter::setFileStartOffset`，同样在 `writeFrame` 内部、两个调用点自动生效）——这些帧既不在文件里、也不占文件字节，于是文件第一帧的时间戳就是 batch 起点 + N 帧，fxcorr-f 的 `anchorbytes`（"文件首帧到 batch 起点的字节数"）为**负**。跳过的帧号仍然推进，所以文件内容的时间与缺口/filler 无关地正确，错的只能是读法——这正是 `reader-model.md` 4.1 的 A1/A2/A3 三条缺陷此前**只在 t25362 上验证过**、改坏了没有回归可依的那一块（`fxcorr/v4-plan.md` A2）。判据是 `check_reader.py` 的 E5 绝对时间锚，检验步骤见 `fxcorr/test/gaps/run_startoffset.sh`；不设或 `0` = 行为完全不变。
