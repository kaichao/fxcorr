...
- 读 `workdir/fengine/<batch_id>/batch.json`（make_testdata.sh 预写），取 start_mjd / n_subints / config_file。
...
- **帧对齐三重校验**（数据连续性，分布生成的前提）：batch 起点须 subint 边界（1µs 容差，同 fxcorr-f）→ 整秒 snap（1µs 容差）→ 帧边界整除；batch 时长须帧长整数倍。VDIF 帧号按秒回绕（0..fps-1），帧号连续性由起点约束保证。
