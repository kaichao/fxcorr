...
| `FXSIM_GAPS` | 关 | **记录中断**（缺口/filler 处理回归，检验步骤见 `test/gaps/README.md`）：逗号分隔 `<秒>:<帧数>[:f]`——在距 batch 起点该秒处让帧号跳过 `<帧数>`（真实丢失的帧）；`:f` 改为 filler 形式，额外写入同样多的**帧头全零**帧占位（占文件字节、不占时间轴，见 `fxcorr/reader-model.md`）。两种形式在**同一位置**造出同一中断、**帧号范围相同**，与 t25362 的真实中断同构（同一观测中一个 datastream 用 filler、其余用缺帧）。legacy 与新路径都生效；不设 = 行为完全不变 |
