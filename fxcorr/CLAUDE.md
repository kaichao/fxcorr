...
- **v4-plan.md**：V4 计划——读取路径改进路线（阶段 A 资产补齐 / B 修 E4 窗口语义 / C 分层重构 / D 判据固化，每节点一个 commit）、验收线、规划条件评估、真实数据获取策略。**改 reader 的计划与顺序时改它**（分析与判据仍在 reader-model.md，本文件不重复）。
