...
- **文件起点语义**：file-per-batch 布局下 raw 文件的时间起点**不要求**等于 batch 起点——真实观测中各记录系统的帧计数器不同相，文件可以从某一秒的中途开始（t25362 实测：BA 从秒内第 1269 帧开始、比 batch 起点晚 79.312 ms，S6 恰在秒边界）。VDIF 文件的时间起点由**首帧时间戳**确定，fxcorr-f 据此算 `anchorbytes` 吸收该距离（做法同 Mk5B/LBA 路径）；起点之前（batch 头或 subint 区间）无数据的部分判为无效。batch.json 的 start_mjd 是 batch 起点的精确表示。文件中间缺帧（记录中断）同样受支持：f 侧按 VDIF 帧号的连续性检测缺口、修正读取位置并把缺口处的块判无效（实现见 `applications/fxcorr-f/CLAUDE.md`）。
...
- **文件内缺帧的表现**：帧号在缺口处跳跃（缺 N 帧即步进 N+1），文件的字节流因此比时间轴短；缺口之后，按时间线性换算出的字节位置会**偏后**于数据实际所在位置。实测 t25362 的 BA 每个 ds 有 7–8 处、单处缺 1–64 帧（合计约 145 帧/12 秒），S6 无中间缺口、仅部分 ds 在文件末尾少 1 帧。
- **文件内中断的第二种形态：filler 帧**：记录中断处写入的数据块并非只有"直接缺帧"一种——记录系统也可能写入**帧头全零**（word0/word1/word2 全零，于是 invalid 位=0、帧长度字段=0）的帧来占位。这类帧**占用文件字节、不占用时间轴**：判据是段后帧号只跳过真实丢失的帧数（t25362 的 BA thread 2 实测：段前帧号 4053、段后 4064，只缺 10 帧，而该段有 81 帧）。因此同一份数据里两种形态可能并存——t25362 的 BA 有 8 个 datastream 用"直接缺帧"（各缺 144–148 帧）、1 个用 filler 占位（1162 帧／8 段），S6 则完全没有中断。**定位后果与缺口相反**：缺口让字节流**短**于时间轴（按时间线性换算的位置**偏后**，需减去已过去的缺口字节），filler 让字节流**长**于时间轴（位置**偏前**，需加上已越过的 filler 字节），filler 覆盖的时间段判为无效。填充帧的帧头本身就足以识别，不需要额外元数据。**注意 filler 段的两端往往同时夹着真实缺口**（记录中断在段的两端各留下一处）：缺口落在被"位置前移"让出的区段里，必须与 filler **分别计数**——只数 filler 而漏掉缺口，会让该流之后的读取位置持续偏后、数据整体错位（t25362 的 BA ds_2 实测 **143 帧缺口 + 1162 帧 filler**，修复细节见 `applications/fxcorr-f/CLAUDE.md`）。
- **两种定位模型的差异**（同一份数据、同样的配置，为什么上游与 fxcorr-f 的路径不同）：`mpifxcorr` 从文件开头**顺序读**，每段数据的起始时间取自**数据内的帧时间戳**，"文件起点相对 batch 起点的偏移"因此是自动获得的，缺帧也由 `vdifmux` 按帧号成流、在缺口处补无效帧，天然免疫；`fxcorr-f` 按 `字节位置 = (时间 − batch 起点) × 速率` 的线性模型**定位读取**，所以偏移（anchorbytes）与缺口修正量都必须显式求出。给 fxcorr-f 喂数据时，这两个量由文件自身的时间戳推出，不需要额外元数据。
- **对拍失败的症状 → 病因对照（t25362 实测，2026-09-16/17）**：真实数据与 mpifxcorr 基准对不上时，差异的**覆盖形态**直接指向病因，不必从"可见度差 1%"一路倒推到底层定位。下表是 t25362 一个观测（BA/S6 两站、11.264 s、BA 每站 8 个 datastream）暴露的全部缺陷，按症状排列：

  | 症状（差异覆盖面） | 病因 | 实测数字 |
  |---|---|---|
  | 该站参与**所有**记录（含无缺口积分）可见度差 ~1%、weight 差 0.4%，**另一站完全一致** | 文件起点偏移未算进读取位置（`anchorbytes` 缺失）——该站全部数据按线性时间读偏 | 锚点偏一帧 = 62.5 µs |
  | PCAL 里**序号为奇数的 tone 全部相位翻转**（每个 ds、每个积分都是"隔一个反号"） | 同上，但错在取整：`anchorbytes` 为负时用 C++ 截断（= 向上取整）而非 `floor(x+0.5)`，锚点整偏一帧；62.5 µs 比 5 MHz pcal 周期的整数倍恰好差 100 ns | 16 kfps 下一帧 = 62.5 µs |
  | `missing frames` 虚增数百倍 | 把 **filler 帧**的帧号跳跃当成缺口：filler 帧头全零使帧号回到 0，按模 fps 算出一个整秒的假缺失 | 报 64066，真值 143（448 倍） |
  | `missing frames` 虚增到几万，读取位置前移累积到数百 MB | 发现 filler 后按"跳过距离 = 全 filler"推断，但跳过的区段里还跟着数据帧 → 下一次前移更多、正反馈失控。必须**实扫该区段逐帧判定** | 虚增到 44650；前移 514 MB／4 秒 |
  | `missing frames` 虚增 3–4 倍 | 同一缺口被相邻 subint **重复计入**。去重键必须用**修正后**的读取位置（帧号每秒回绕不可比，未修正位置差整数个 subint） | 145 虚增到 536 |
  | filler 扫描漏计 | 读取位置前移会**跳过**一段从未扫描的文件（长度恰等于刚发现的 filler 字节），其中的 filler 帧必须补扫 | 只计到 626／1162 |
  | **某个 ds 参与的所有 frq** 从第一个 filler 段起可见度错位，**其他 ds 完全正常** | filler 帧未被识别，该 ds 的读取位置从第一个 filler 段起持续偏移 | — |
  | weight 全局塌陷（缺口之后的所有 subint） | `gapinvalid` 未按 subint 重建，缺口位置的块被永久清掉 | — |
  | 缺口之后的数据"读到了、且全部标为有效" | 缺口不能用线性时间换算：字节流比时间轴短，线性换算的位置偏**后**于数据实际位置 | — |
  | 跨 buffer 边界报出大量**假**缺口 | 缺口只在 buffer **内部**找——跨 buffer 的帧号步进是"读取位置按 subint 长度推进、帧号按帧长推进"的正常漂移 | 合成无缺口数据实测 ±4 帧 |
  | 某个 ds 参与的**所有**组合可见度分量**符号相反、数值无关**（rel 3–5e-2，而 **weight 完全相同、都 1.0**），同站其他 ds 与另一站全部正常 | **filler 段边界/被跳过区段里的真实缺口漏检**：缺口落在"位置前移"让出的区段、或补扫区段与 buffer 的接缝上，主循环与补扫两边都不检测 → 该 ds 读取位置持续偏后，数据整体错位 | 真值 143 帧只记 2 帧，偏后 70 帧 = 4.375 ms |
  | 同上但 `missing frames` 虚增到真值的 1.5 倍以上 | 补扫推进了帧号链却没把新链尾存回去，后续每个补扫都从同一个旧帧号重新比较，同一缺口被反复计入 | 143 帧虚增到 252 |

  **排查顺序**：先看 `GAPCHECK summary` 那一行（`filler frames` 应与外部帧号扫描逐一致；t25362 的 BA 实测 8 个 ds 为 0/0/1162/0/0/0/0/0），再按**覆盖面**分流——"只影响一个 ds"指向该 ds 的读取位置链（filler/缺口），"影响该站全部 ds"指向起点偏移（`anchorbytes`），"影响该 ds 全部 frq"指向该 ds 的定位链，"只影响某几个积分"指向 vdifmux 滞后等上游现象（见 v1-plan 2.3）。

- **上游代码的隐含假设（拆分时未识别，是上述缺陷的共同来源）**：mpifxcorr 的数据路径建立在"文件从 batch 起点开始、帧号连续、帧帧有效"三条假设上。这些假设在**上游的顺序读模型下天然成立**（见上条"两种定位模型的差异"），因此既没有被写进上游代码的注释，也没有被写进任何规范——拆分时无从识别。换成 fxcorr-f 的**定位读**模型后，每一条都变成必须显式求解的量：

  - `mark5_unpack_with_offset` 假定"缓冲里的位置 i 就是第 i 帧"——缺口修正必须**整帧后移**缓冲内容，否则解包位置与数据错位；
  - `locate()` 的线性时间换算假定字节流与时间轴等长——缺口（字节流**短**）与 filler（字节流**长**）分别向两个方向破坏它；
  - "读到即有效"的前提——缺口/filler 覆盖的块必须显式记入 `gapinvalid` 并逐块清掉。

  **教训**：从上游拆代码时，凡涉及"位置 ↔ 时间"互换的式子，都要问一遍"如果数据不连续、文件起点不听话，这个式子还成立吗"；答案是否定的地方就是必须新增的显式修正量。**合成数据全绿不等于真实数据能跑对**：`fxcorr/test/` 的全部特性测试（P0-P11）都用 fxcorr-sim 生成的理想数据（帧连续、起点恰在 batch 起点、无 filler），上述病态情况在 t25362 之前**从未被测试覆盖**。
