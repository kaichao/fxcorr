...
| A3 | 把 `run_filler.sh` / `run_boundary.sh` 的判据接到 `check_reader.py`（相对判据 → 绝对判据的入口收敛） | 两套资产退出码一致；旧判据不删，仍作交叉核对 |
...
1. B1 的语义定案未完成——"读取窗口应该多长"尚无定义;
2. A2 的起点偏移资产不存在——动结构时 A1–A3 无回归可依;
3. A3 的判据入口未收敛——现在两套判据并存。
