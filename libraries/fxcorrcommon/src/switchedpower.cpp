...
SwitchedPower::SwitchedPower(const Configuration * conf, int configindex, int dsindex)
{
	init();

	// upstream constructs with (conf, mpiid) and datastreamId = mpiid-1;
	// here the 0-based datastream index is passed in directly
	datastreamId = dsindex;
	filepath = conf->getOutputFilename();
	startMJD = conf->getStartMJD();
	startSeconds = conf->getStartSeconds();
	frequency = conf->getDSwitchedPowerFrequency(dsindex);

	// build the format name for this datastream (vdiffile.cpp:568; with
	// nummuxthreads == 1 the vdifmux output frame size is the input frame size)
	int nchan = conf->getDNumRecordedBands(configindex, dsindex);
	double bw = conf->getDRecordedBandwidth(configindex, dsindex, 0);
	int nbits = conf->getDNumBits(configindex, dsindex);
	Configuration::datasampling sampling = conf->getDSampling(configindex, dsindex);
	int framebytes = conf->getFrameBytes(configindex, dsindex);
	int decimation = conf->getDDecimationFactor(configindex, dsindex);
	int alignment = conf->getDAlignmentSeconds(configindex, dsindex);
	int nthreads = conf->getDNumMuxThreads(configindex, dsindex);
	if(conf->genMk5FormatName(conf->getDataFormat(configindex, dsindex), nchan, bw, nbits, sampling,
		framebytes, decimation, alignment, nthreads, formatname) != 1)
	{
		cerr << "SwitchedPower: cannot generate mark5 format name for datastream " << dsindex << endl;
		failed = true;
	}
}
...
// take one block of raw bytes and accumulate its switched power statistics.
// the block must start on a frame boundary: mark5access reads the frame time
// and channel count from the frame headers, so nothing else is needed here.
int SwitchedPower::feed(u8 *buffer, int nbytes)
{
	if(failed)
	{
		return 0;
	...
	struct mark5_stream *m5stream = new_mark5_stream_absorb(
		new_mark5_stream_memory(buffer, nbytes),
		new_mark5_format_generic_from_string(formatname) );
	if(m5stream)
	{
		mark5_stream_fix_mjd(m5stream, startMJD);
		feedMark5Stream(m5stream);
...
int SwitchedPower::feedMark5Stream(mark5_stream *ms)
{
	int mjd, sec;
	double ns;
	int status;
